...
namespace CombatAndroid::ECS {
    ...
    public:
        ...
        //! シャドウパスのカリングを無効にして起動するよう予約します。
        //! @note  コマンドライン引数 --no-shadow-cull を見たWinMainが呼ぶ。
        //!        カスケードシャドウの間引きがどれだけ効いているかを
        //!        同じビルドで比較するための口（実行中はF5でも切り替えられる）
        static void RequestShadowCullingDisabled();
    ...
    private:
        ...
        bool m_dense = true;
        bool m_shadowCulling = true;    //!< シャドウパスのカリングが有効か（F5で切り替え）
